...
source_languages = [["english", "tts_models/en/ljspeech/vits"]] #, ["francais", "tts_models/fr/css10/vits"]]
...
for json_file in json_files:
    ...
    with open(file_path, 'r') as file:
        ...
        for source_language in source_languages:
            tts = TTS(source_language[1])
            
            language_name = data['language_name']
            language_slug = data['slug']
            print(f"\n--- Processing {source_language[0]} {language_name} ({json_file}) ---\n")
            ...
            for i, sentence in enumerate(data['sentences']):
                if source_language[0] not in sentence:
                    break

                translated_sentence = sentence[source_language[0]]
                ...
                print(f"Generating audio for: '{translated_sentence}'")
                ...
                tts.tts_to_file(text=translated_sentence, file_path=tts_output_filepath)
                translated_tts_audio = AudioSegment.from_file(tts_output_filepath)
            ...
            output_filename = f"{source_language[0]}_{language_slug}_course.wav"
            ...
            html_content += f"""<div class="audio-item">
                <h2>{source_language[0].capitalize()} to {data['language_name']} course</h2>
                <audio controls>
